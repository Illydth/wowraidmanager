...
elseif(($_GET['mode'] == 'new' || $_GET['mode'] = 'edit') && isset($_POST['submit']))
{
	...
	elseif($_GET['mode'] == 'edit')
	{
		...
		$sql = sprintf("UPDATE " . $phpraid_config['db_prefix'] . "announcements SET visible=%s,title=%s,message=%s WHERE announcements_id=%s", quote_smart($visible), quote_smart($title),quote_smart($message),quote_smart($id));
		$db_raid->sql_query($sql) or print_error($sql, $db_raid->sql_error(), 1);
	}
	header("Location: announcements.php?mode=view");
}
