...
function step5($auth_type)
{
	...
	else if ($step5_substep == "done")
	{
		...
		$sql = "SELECT username FROM " . $phpraid_config['db_prefix']. "profile WHERE username = '$iums_useradmin_name'";
		...
		{
			$sql = "INSERT INTO " . $phpraid_config['db_prefix'] . "profile ( `username`, `email`,`password`, `priv`) VALUES('$iums_useradmin_name','$iums_useradmin_email','$iums_useradmin_password','1')";
			$result = mysql_query($sql) or die("Error inserting " . mysql_error());
		}
		else
		{
			$sql = "UPDATE " . $phpraid_config['db_prefix'] . "profile SET priv='1' WHERE username='$iums_useradmin_name'";
