...
//		$content .= '<input type="hidden" name="e107_useradmin_email" value="'.$e107_useradmin_email.'" class="post">';
		...
		$sql = printf("SELECT user_loginname, user_email, user_id FROM ".$e107_prefix."user WHERE user_loginname = %s", quote_smart($e107_useradmin_name));
		...
		$sql = printf("SELECT username FROM " . $phpraid_config['db_prefix']. "profile WHERE username = %s", quote_smart($e107_useradmin_name));
		...
		{
			// update
			$sql = printf("UPDATE " . $phpraid_config['db_prefix'] . "config SET config_value=%s WHERE config_name='e107_base_path'", quote_smart($e107_base_path));
			mysql_query($sql) or die("Error updateing e107_base_path in config table. " . mysql_error());
			$sql = printf("UPDATE " . $phpraid_config['db_prefix'] . "config SET config_value=%s WHERE config_name='e107_table_prefix'", quote_smart($e107_prefix));
			...
			$sql = printf("UPDATE " . $phpraid_config['db_prefix'] . "config SET config_value=%s WHERE config_name='e107_auth_user_class'", quote_smart($e107_auth_user_class));
			mysql_query($sql) or die("Error updateing e107_auth_user_class in config table. " . mysql_error());
			$sql = printf("UPDATE " . $phpraid_config['db_prefix'] . "config SET config_value=%s WHERE config_name='alt_auth_user_class'", quote_smart($alt_auth_user_class));
		...
		else
		{
			...
			mysql_query($sql);
			$sql = printf("INSERT INTO " . $phpraid_config['db_prefix'] . "config VALUES('e107_base_path',%s)", quote_smart($e107_base_path));
			mysql_query($sql) or die("Error inserting e107 Base Path in config table.");
			$sql = printf("INSERT INTO " . $phpraid_config['db_prefix'] . "config VALUES('e107_table_prefix',%s)", quote_smart($e107_prefix));
			mysql_query($sql) or die("Error inserting e107_table_prefix in config table. " . mysql_error());
			// Insert the e107_auth_user_class
			$sql = printf("INSERT INTO " . $phpraid_config['db_prefix'] . "config VALUES('e107_auth_user_class', %s)", quote_smart($e107_auth_user_class));
			mysql_query($sql) or die("Error inserting e107_auth_user_class in config table. " . mysql_error());
			// Insert the e107_auth_user_class
			$sql = printf("INSERT INTO " . $phpraid_config['db_prefix'] . "config VALUES('alt_auth_user_class', %s)", quote_smart($alt_auth_user_class));
